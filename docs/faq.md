...
??? question "Excel is reformatting my date values — what should I do?"
    This is a known Excel issue with date-like strings (e.g. `2023-01-01`). We recommend filling out your templates using a script (Python, R) or Google Sheets to avoid this.

## Validation results

??? question "What's the difference between an error and a warning?"
    **Errors (❌)** must be fixed — the sanitized CSV download will be blocked until they are resolved. **Warnings (⚠️)** are recommended fixes but you can proceed without them if you have a valid reason (use the comment box to explain).
