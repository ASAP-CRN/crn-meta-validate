...
|---|---|---|---|---|---|
| **ASSAY.csv** | comma | 3 | `sequencing_end`, `RIN` | Non-compliant values:<ul><li>`'dual'` → `Single`, `Paired-end`</li><li>`'good'` → float, `NA`, or a FillNull value</li></ul> | <ul><li>Non-compliant Enum and Float values.</li><li>Valid semicolon-delimited multi-value entry (`instrument` = `10x_chromium_x;illumina_novaseq_6000`).</li></ul> |
